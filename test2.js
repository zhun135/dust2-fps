...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    html, body { width: 100%; height: 100%; overflow: hidden; background: #000; user-select: none; font-family: 'Segoe UI', sans-serif; }
    canvas { display: block; }
    #crosshair { position: fixed; top: 50%; left: 50%; transform: translate(-50%,-50%); pointer-events: none; z-index: 10; width: 2px; height: 2px;
      box-shadow: 0 -6px 0 rgba(0,255,0,0.7), 0 6px 0 rgba(0,255,0,0.7), -6px 0 0 rgba(0,255,0,0.7), 6px 0 0 rgba(0,255,0,0.7); }
    #hud { position: fixed; bottom: 30px; right: 30px; color: #fff; font-size: 18px; z-index: 10; text-align: right; text-shadow: 1px 1px 3px #000; }
    #ammo { font-size: 26px; font-weight: bold; color: #fc0; }
    #health { margin-top: 4px; color: #0f0; font-size: 15px; }
    #armor { margin-top: 2px; color: #88ccff; font-size: 15px; }
    #info { position: fixed; top: 50%; left: 50%; transform: translate(-50%,-50%); color: #fff; font-size: 14px; z-index: 20; text-align: center;
      background: rgba(0,0,0,0.75); padding: 16px 28px; border-radius: 8px; letter-spacing: 1px; }
    .hidden { display: none !important; }
    #debug { position: fixed; top: 8px; left: 8px; color: #0f0; font-size: 11px; z-index: 30; font-family: monospace; pointer-events: none; line-height: 1.5; }
    #deathScreen { position: fixed; inset: 0; background: rgba(180,0,0,0.7); z-index: 50; display: none; flex-direction: column;
      align-items: center; justify-content: center; color: #fff; font-size: 24px; font-weight: bold; }
    #deathScreen.show { display: flex; }
    #deathScreen button { margin-top: 24px; padding: 14px 40px; font-size: 20px; cursor: pointer;
      background: #c0392b; color: #fff; border: 2px solid #e74c3c; border-radius: 8px; font-weight: bold; }
    #deathScreen button:hover { background: #e74c3c; }
    #ammoHint { position: fixed; bottom: 100px; left: 50%; transform: translateX(-50%); color: #0f0; font-size: 16px;
      z-index: 25; background: rgba(0,0,0,0.6); padding: 6px 18px; border-radius: 6px; display: none; pointer-events: none; }
  </style>
</head>
<body>
  <div id="crosshair"></div>
  <div id="hud"><div id="ammo">30 / 90</div><div id="health">HP 100</div><div id="armor">馃洝 0</div></div>
  <div id="info"><p>馃柋锔?鐐瑰嚮鐢婚潰寮€濮?br>WASD 绉诲姩 路 Space 璺宠穬 路 Shift 濂旇窇 路 宸﹂敭灏勫嚮 路 R 鎹㈠脊</p></div>
  <div id="debug"></div>
  <div id="deathScreen"><p>馃拃 浣犻樀浜′簡</p><button id="respawnBtn">閲嶆柊閮ㄧ讲</button></div>
  <div id="ammoHint">鎸?F 琛ュ厖寮硅嵂</div>
  ...
  <script type="module">
    ...
    function building(x1, z1, w, d, h, wallMat, floorMat) {
      ...
      // 鍥涢潰澧?      W(w, h, 0.4, wallMat, x, hy, z1);           // 鍓?      W(w, h, 0.4, wallMat, x, hy, z1 + d);        // 鍚?      W(0.4, h, d, wallMat, x1, hy, z);            // 宸?      W(0.4, h, d, wallMat, x1 + w, hy, z);        // 鍙?      // 灞嬮《
    ...
    function buildingWithDoor(x1, z1, w, d, h, wallMat, floorMat, doorSide, doorW, doorH) {
      ...
      // 鍏朵粬涓夐潰澧欏畬鏁?      if (doorSide !== 'left')  W(0.4, h, d, wallMat, x1, hy, z);
    ...
    // 涓矾 鈫?CT 鏂瑰悜澧?    W(0.5, 3.5, 16, mat.wallB, -14, 1.75, -1);
    // 涓矾 鈫?B 杩炴帴澧?    W(0.5, 3.5, 14, mat.wallB, -19, 1.75, -16);

    // ================ Long A (A澶ч亾) ================
    F(42, 10, mat.floorLongA, 2, -0.05, 16);
    // 涓や晶寤虹瓚澧欙紙妯℃嫙琛楁櫙锛?    W(42, 3.5, 0.5, mat.wallA, 2, 1.75, 11);     // 涓嬩晶
    W(42, 3.5, 0.5, mat.wallA, 2, 1.75, 21);     // 涓婁晶
    // 绐楁埛瑁呴グ 鈥?涓婁晶澧?    for (let i = 0; i < 5; i++) {
      const wx = -15 + i * 8;
    ...
    // 涓?A 骞冲彴鏈€鍚庡嚑绾?    for (let i = 0; i < 3; i++) {
    ...
    // ================ B 娲?(B Tunnels) ================
    // 闅ч亾宸﹀彸澧?+ 椤剁洊妯℃嫙闅ч亾鎰?    W(0.5, 3.5, 22, mat.darkWall, 8, 1.75, -6);
    W(0.5, 3.5, 22, mat.darkWall, 19, 1.75, -6);
    // 闅ч亾椤?    for (let i = 0; i < 6; i++) {
    ...
    W(0.3, 1.5, 6, mat.wallA, -18, 1.1, -30); // 宸?    // 鎺╀綋
    ...
    // 棰勮宸￠€昏矾鐐?    const patrolRoutes = [
      // CT 澶栫┖鍦?      [{ x: -42, z: -20 }, { x: -45, z: -10 }, { x: -38, z: -8 }],
      // 涓矾 CT 渚?      [{ x: -10, z: 0 }, { x: -18, z: 5 }, { x: -12, z: -3 }],
      // 涓矾 T 渚?      [{ x: 10, z: -2 }, { x: 15, z: 5 }, { x: 8, z: -5 }],
      // T 澶栫┖鍦?      [{ x: 42, z: -20 }, { x: 48, z: -14 }, { x: 38, z: -10 }],
      // 鍗椾晶鍚庤
      ...
      // B 鍖哄鍥?      [{ x: -8, z: -25 }, { x: -2, z: -22 }, { x: -10, z: -20 }],
    ];
    ...
    function createEnemy(x, z, routeIdx) {
      ...
      // 澶?      const head = new THREE.Mesh(new THREE.SphereGeometry(0.16, 8, 8), skin);
      ...
      // 鑵?      const legL = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.4, 0.12), dark);
    ...
    function createMeleeEnemy(x, z, routeIdx) {
      ...
      // 鏇村．鐨勮韩浣?      const body = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.6, 0.35), skin);
      ...
      // 澶ф嫵澶?      const fistL = new THREE.Mesh(new THREE.SphereGeometry(0.12, 6, 6), dark);
    ...
    // 鐢熸垚 5 涓繎鎴樻晫浜?    [[-20, 5, 0], [15, -15, 1], [-5, -22, 2], [-55, -14, 0], [48, -20, 5]].forEach(([x, z, r]) => enemies.push(createMeleeEnemy(x, z, r)));
    ...
    function updateEnemies(dt) {
      ...
      for (const en of enemies) {
        ...
        // ---- 绉诲姩閫熷害 ----
        if (en.state === 'patrol') {
          // 鍚戝贰閫昏矾鐐圭Щ鍔?          const wp = en.route[en.patrolIdx];
        ...
        for (const b of boxes) {
          // X 杞存娴?          if (nx + eR > b.min.x && nx - eR < b.max.x &&
              ez + eR > b.min.z && ez - eR < b.max.z &&
              eFoot < b.max.y && eHead > b.min.y) freeX = false;
          // Z 杞存娴?          if (ex + eR > b.min.x && ex - eR < b.max.x &&
        ...
        raycaster.set(new THREE.Vector3(g.position.x, g.position.y + 1.0, g.position.z), new THREE.Vector3(0, -1, 0));
        const ghits = raycaster.intersectObjects(wallMeshes, false);
        if (ghits.length > 0 && ghits[0].distance < 2.0) {
          const targetY = ghits[0].point.y;
          g.position.y += (targetY - g.position.y) * Math.min(dt * 12, 1);
        }
      }
    }
    ...
    function updateArmorPacks(dt) {
      ...
      // 鎷惧彇妫€娴?      const cp = camera.position;
    ...
    // F 閿ˉ鍏呭脊鑽?    const ammoCratePos = new THREE.Vector3(3, 0.5, 22);
    ...
    function shoot() {
      ...
      for (const en of enemies) {
        ...
        if (ehits.length > 0) {
          ...
          en.hp -= dmg;
          hitEnemy = true;
          // 琛€杩?          const blood = new THREE.Mesh(new THREE.SphereGeometry(0.05, 4, 4), new THREE.MeshBasicMaterial({ color: 0xcc0000 }));
